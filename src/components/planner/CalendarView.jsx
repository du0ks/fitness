...
export default function CalendarView() {
    ...
    const [selectedDate, setSelectedDate] = useState(null); // For assigning workout to specific date
    const [isManageMode, setIsManageMode] = useState(false); // To add recurring rules
        ...
        <div className="p-4 safe-top pb-24 space-y-6">
            ...
            <div className="bg-zinc-900 rounded-3xl p-4 border border-zinc-800">
                ...
                <div className="grid grid-cols-7 gap-y-2">
                    {days.map(day => {
                        const workoutId = getWorkoutForDate(day);
                        ...
                        return (
                            <div
                                ...
                                className={clsx(
                                    ...
                                    isToday(day) && "bg-zinc-800 text-white",
                                    // Hover effect
                                    "hover:bg-zinc-800/50 cursor-pointer"
                                )}
                            >
                                <span className={clsx("text-sm", isToday(day) && "font-bold text-blue-500")}>{format(day, 'd')}</span>
                                {workout && (
...
function ManageHabitModal({ isOpen, onClose, workouts, onAdd }) {
    const [type, setType] = useState('weekly'); // weekly | interval
